...
for train in train_sizes:
    ...
    if use_vals:
        ...
        for i, (pt, (ids, wts)) in enumerate(zip(pts, output)):
            ...
            a = train_points[ids[1:]] - train_points[ids[0]]
            s = np.linalg.svd(a, full_matrices=False, compute_uv=False, hermitian=False)
            smallest_sing.append( s[-1] )
            # Compute the error.
            prediction_errors.append(abs(test_values[i] - sum(train_values[ids] * wts)))

        vertex_distances.append( nearest_vertex )
        edge_lengths.append( longest_edge )
        sing_values.append( smallest_sing )
        errors.append( prediction_errors )
    t.stop()
    print(f"    {t()} seconds")
...
if use_vals:
    ...
    with open(f"Delaunay-abs_error-oscillatory-{dimension}D-{test_size}N.csv","w") as f:
        print(",".join(["Number of Points"] + [f"Sample {i}" for i in range(test_size)]), file=f)
        for t,v in zip(train_sizes,errors):
            if len(v) > 0: print(",".join(list(map(str,[t]+v))), file=f)
    with open(f"Delaunay-nearest_vertex-oscillatory-{dimension}D-{test_size}N.csv","w") as f:
        print(",".join(["Number of Points"] + [f"Sample {i}" for i in range(test_size)]), file=f)
        for t,v in zip(train_sizes,vertex_distances):
            if len(v) > 0: print(",".join(list(map(str,[t]+v))), file=f)
    with open(f"Delaunay-longest_edge-oscillatory-{dimension}D-{test_size}N.csv","w") as f:
        print(",".join(["Number of Points"] + [f"Sample {i}" for i in range(test_size)]), file=f)
        for t,v in zip(train_sizes,edge_lengths):
            if len(v) > 0: print(",".join(list(map(str,[t]+v))), file=f)
    with open(f"Delaunay-smallest_sv-oscillatory-{dimension}D-{test_size}N.csv","w") as f:
        print(",".join(["Number of Points"] + [f"Sample {i}" for i in range(test_size)]), file=f)
        for t,v in zip(train_sizes,sing_values):
            if len(v) > 0: print(",".join(list(map(str,[t]+v))), file=f)
    p.add_box(f"Absolute Errors", errors, [i for i in range(1,len(train_sizes)+1)])
    ...
    p.add_box(f"Smallest Singular Values", sing_values, [i for i in range(1,len(train_sizes)+1)])
