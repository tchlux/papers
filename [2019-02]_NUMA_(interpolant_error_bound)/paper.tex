...
where \changed{$W_k(x^{(k)}) = 1$, $W_k(x) = \bigl(\bigl(r_k - \bigl\|x -
x^{(k)}\bigr\|_2\bigr)_+ \big/ \bigl( r_k \bigl\|x - x^{(k)}\bigr\|_2 \bigr)\bigr)^2$}{}
and $r_k \in \mathbb{R}$ is the smallest radius such that at least
$d+1$ other points $x^{(j)}$, $j \not = k$, are inside the closed
Euclidean ball of radius $r_k$ about $x^{(k)}$. The computational
complexity of ShepMod is $\mathcal{O}(n^2d)$. This paper uses a
Fortran 95 implementation of ShepMod derived from SHEPPACK
...
\begin{figure}
  ...
  \caption{\color{red} On the left above is a depiction of the radius
    of influence for three chosen points of a collection using the
    modified Shepard criteria. On the right is the shape of the weight
    function $W_k$ for all data points, where $W_k(x) = 0$ when
    $\|x-x^{(k)}\|_2 \geq r_k$. It can also be seen that in the limit
    as $W_k$ goes to infinity at $x_k$, the relative weight of $x_k$
    goes to one.
  \vspace{-.1cm}}
  \label{fig:shepard}
\end{figure}
...
\cite{thacker2010algorithm}. The interpolant has the form
 ...
 {\sum\limits_{k=1}^{n}W_k(x)} ,$$
where $W_k(x)$ is \changed{the same as for the modified Shepard method}{a locally supported weighting function} and $P_k(x)$
...
\begin{figure}
  ...
  \caption{\color{red} Above is a depiction of the Voronoi cell
    ...
    (dots). In this example, the Voronoi model basis function about
    the center most point has nonzero weight in the shaded region and
    transitions from a value of one at the point to zero at the
    boundary of the twice expanded Voronoi cell (solid line).
  \vspace{-.1cm}}
...
example of the \changed{end-to-end}{round-trip} prediction methodology from known and
...
signal-to-noise (SNR) ratio of 10:1 (a regression problem).
...
number of \changed{well-spaced}{} data points should result in a
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \node[below=of img, node distance=1cm, yshift=1cm] {Number of Data Points};
    \node[left=of img, node distance=0cm, rotate=90, anchor=center,yshift=-0.7cm] {Euclidean Distance};
  \end{tikzpicture}
  \caption{\color{red}The distribution of absolute error, distance to
    the nearest data point, smallest singular value and the longest
    edge of the simplex containing each approximation point in the
    tests from Fig. \ref{fig:convergence-2d} and
    ...
    data points. This is seen by the relatively small reduction in
    $k$, and this explains the slow reduction in error seen by
    Delaunay in Fig. \ref{fig:convergence-20d}.}
  \label{fig:data-spacing}
\end{figure}
}{}

\changed{The results achieved for both the $d = 2$ and $d = 20$ align
  with the theoretical error bound as can be seen in Figure
  ...
  reduction in data sparsity. The same result holds for a Latin
  hypercube well-spaced point set, suggesting that sparsity is an
  inherent problem in high dimension approximation problems. This
  result is not a product of the chosen test function, but rather the
  exponential increase in complexity that accompanies increased
  dimension.}{}

In summary, the regime of interpolation is \changed{}{much} greater
for moderate to high dimensional problems \changed{due to increased
  sparsity}{}, even for functions with random noise incorporated into
their evaluation. Acknowledging the viability of interpolation for
problems of moderate dimension, the next section will consider
real-world problems of similar proportion (thousands of examples in
tens of dimensions).
