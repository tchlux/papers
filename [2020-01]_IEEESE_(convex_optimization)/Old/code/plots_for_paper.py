...
plot_in_2D = True
plot_in_3D = False
add_test_points_to_3D = plot_in_3D and False
...
for func in FUNCTIONS:
    ...
    if plot_in_2D:
        ...
        p.add_func(name, lambda x: func(array([x], dtype=float)),
                   [func.lower,func.upper], color=p.color(1))
        # Make the derivative plot.
        g = Plot("", f"<i>{name} derivative</i>", "",
                 font_family="Times", font_size=16)
        g.add_func(f"{name} derivative",
                   lambda x: func.grad(array([x],dtype=float)),
                   [-1,1], color=p.color(0))
        # Plot the two together.
        multiplot([[p, g]], show_legend=False,
                  file_name=f"{name}-2D.html",
