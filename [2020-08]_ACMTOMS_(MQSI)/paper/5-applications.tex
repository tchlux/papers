...
Comparisons between this work and lower order spline algorithms are
problematic, especially with sparse data. For exact data from smooth
functions, comparing derivative values, higher order methods always win
as the data density increases. This result is demonstrated in the work of
Lux et al. [2020]. For sparse or synthetic data, what constitutes a
better fit is either subjective (e.g., less wiggle) or dependent on the
specific problem. If an application demands $C^2$ continuity and
monotonicity, the target of {\tt MQSI}, then this software provides such
solutions while no existing published spline algorithms suffice.
